...
function executeWithWebGLTexture(execution, input, output) {
  const inputTexture = tf.backend().getTexture(input.dataId);
  const gl = tf.backend().getGPGPUContext().gl;
  const inputTexShape = tf.backend().getDataInfo(input.dataId).texShape;
  execution.setInput(0, gl, inputTexture, inputTexShape[0], inputTexShape[1]);
  
  const outputTexture = tf.backend().getTexture(output.dataId);
  const outputTexShape = tf.backend().getDataInfo(output.dataId).texShape;
...
async function tfConv2d(inputDims, filterDims){
  ...
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d/add/relu (WebGL): ${elapsedTime} ms <br/>`;
  console.log(result);
}
...
async function WebNNConvGPUWithTf(inputDims, filterDims) {
  ...
  // warm up
  executeWithWebGLTexture(execution, input, output);
  ...
  const elapsedTime =((performance.now() - start) / iterations).toFixed(2);
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d (WebNN) -> WebGLTexture -> add/relu (WebGL): ${elapsedTime} ms <br/>`;
  console.log(result);
}
...
async function WebNNConvCPUWithTf(inputDims, filterDims) {
  ...
  const elapsedTime =((performance.now() - start) / iterations).toFixed(2);
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d (WebNN) -> ArrayBufferView -> add/relu (WebGL): ${elapsedTime} ms <br/>`;
  console.log(result);
}
...
async function tfConv2dx2(inputDims, filterDims){
  ...
  const elapsedTime = ((performance.now() - start) / iterations).toFixed(2);
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d/add/relu x2 (WebGL): ${elapsedTime} ms <br/>`;
  console.log(result);
}
...
async function WebNNConvGPUx2Model(inputDims, filterDims) {
  ...
  const elapsedTime =((performance.now() - start) / iterations).toFixed(2);
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d/add/relu (WebNN) -> WebGLBuffer -> conv2d/add/relu (WebNN): ${elapsedTime} ms <br/>`;
  console.log(result);
}
...
async function WebNNConvGPUx2WithTf(inputDims, filterDims) {
  ...
  const elapsedTime =((performance.now() - start) / iterations).toFixed(2);
  document.getElementById('output').innerHTML += `Test${testNum++} - conv2d/add/relu (WebNN) -> WebGLBuffer -> conv2d/add/relu (WebGL): ${elapsedTime} ms <br/>`;
  console.log(result);
}

async function test() {
  document.getElementById('backend').innerText = `TF.js sets backend as WebGL`;
